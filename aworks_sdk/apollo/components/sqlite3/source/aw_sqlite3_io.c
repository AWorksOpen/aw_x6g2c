...
static int __sqlite3_io_close (sqlite3_file *id)
{
    ...
    if (rc) {
        pfile->fd = 0;
    } else if ((pfile->delete_flag) && (pfile->path)) {
        aw_unlink(pfile->path);
        sqlite3_free((void *)pfile->path);
    }
...
static int __sqlite3_tempname_get (int nbuf, char **pbuf)
{
    ...
    /*todo: notice!!!!*/
    char *str_temppath = (char *)sqlite3_malloc(nbuf);
    if (str_temppath == NULL) {
        return SQLITE_NOMEM;
    }

    memset(str_temppath, 0, nbuf);
    ...
    memcpy(str_temppath + temppath, "etilqs_", sizeof("etilqs_"));

    j = strlen(str_temppath);

    sqlite3_randomness(15, &str_temppath[j]);

    for (i=0; i<15; i++, j++) {
        str_temppath[j] = (char)chars[((unsigned char)str_temppath[j])%(sizeof(chars)-1)];
    }
...
__fail:
  sqlite3_free((void *)str_temppath);
  return ret;
}
                               ...
                               int          *p_outflags)
{
    ...
    ** automatically deleted. Nor are they ever temporary files.  */
    ...
    if (!__p_filename) {
        ...
        rc = __sqlite3_tempname_get(AW_SQLITE3_MAX_PATH + 2, (char **)&__p_filename);
        if (rc != SQLITE_OK) {
            return rc;
        }
