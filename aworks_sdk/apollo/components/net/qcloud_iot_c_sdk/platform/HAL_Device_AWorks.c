...
static char sg_device_info_file[MAX_SIZE_OF_DEVICE_INFO_FILE + 1] = {"/u/device_info.json"};
...
int HAL_GetDevInfoFromFile(const char *file_name, void *dev_info)
{
    ...
    if (NULL == JsonDoc) {
        Log_e("malloc buffer for json file aw_read fail");
        ret = QCLOUD_ERR_FAILURE;
        goto exit;
    }

    aw_lseek(fd, 0, SEEK_SET);
    rlen = aw_read(fd, JsonDoc, len);
    if (len != rlen) {
...
static int iot_parse_subdevinfo_from_json_file(DeviceInfo *pDevInfo, int *subDevNum)
{
    ...
    fd = aw_open(sg_device_info_file, O_RDONLY, 0777);
    if (fd < 0) {
...
// key device info or cert device info
    ...
    fd = aw_open(sg_device_info_file, O_WRONLY, 0777);
    if (fd < 0) {
        Log_e("open file %s failed", sg_device_info_file);
        return QCLOUD_ERR_FAILURE;
    }

    wlen = aw_write(fd, JsonDoc, strlen(JsonDoc));
