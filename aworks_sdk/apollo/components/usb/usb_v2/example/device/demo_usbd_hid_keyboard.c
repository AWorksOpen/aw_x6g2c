...
static void __task_rev_entry (void *p_arg)
{
    ...
    usbd_keyboard_out_report_t *p_report =
            usb_mem_alloc(sizeof(usbd_keyboard_out_report_t));
    memset(p_report, 0x00, sizeof(usbd_keyboard_out_report_t));

    AW_FOREVER {
        /* 等待接收数据*/
        ret = usbd_hid_keyboard_receive(&__g_usbd_hid, p_report, AW_SEM_WAIT_FOREVER);
        if (ret == AW_OK) {
...
static void task_entry (void *p_arg)
{
    ...
    usbd_keyboard_in_report_t *p_report = NULL;
    ...
    /* 创建一个HID 键盘设备*/
    ret = usbd_hid_keyboard_create(&__g_usbd_obj,
                                   &__g_usbd_hid,
                                   "hid0_keyboard");
    if (ret != AW_OK) {
...
//    AW_TASK_STARTUP(task_rev);
    /* 分配一个键盘上报数据结构内存并初始化*/
    p_report = usb_mem_alloc(sizeof(usbd_keyboard_in_report_t));
    if(p_report == NULL){
        AW_ERRF(("p_report alloc failed\r\n"));
        return;
    }
    memset(p_report, 0x00, sizeof(usbd_keyboard_in_report_t));

    for (;;) {
        /* 相当于键盘的“A”按键*/
        p_report->key[0] = usbd_hid_key_to_code(KEY_A);
        usbd_hid_keyboard_send(&__g_usbd_hid, p_report, 5000);
        /* 松开按键*/
        p_report->key[0] = 0;
        usbd_hid_keyboard_send(&__g_usbd_hid, p_report, 5000);
        aw_mdelay(5000);
