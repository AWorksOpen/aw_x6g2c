...
/* 五个触摸点触摸坐标起始地址*/
const uint16_t GT911_TPX_TBL[5]={GT911_TP1_REG,
                                 GT911_TP2_REG,
                                 GT911_TP3_REG,
                                 GT911_TP4_REG,
                                 GT911_TP5_REG};
...
static void __gt911_address1_init (awbl_gt911_ts_devinfo_t *pinfo)
{
    /* 初始化触摸屏的复位引脚和中断引脚 */
    if(pinfo->rst_pin != -1) {
        int gt911_rst[] = {pinfo->rst_pin};
        if (aw_gpio_pin_request("gt911_rst",
                                 gt911_rst,
                                 AW_NELEMENTS(gt911_rst)) == AW_OK) {
            aw_gpio_pin_cfg(pinfo->rst_pin, AW_GPIO_OUTPUT | AW_GPIO_PULL_UP);
            aw_gpio_set(pinfo->rst_pin, 0);
        } else {
            AW_INFOF(("GT911  GPIO RST pin Request Fail !\r\n"));
        }
    }

    if(pinfo->int_pin != -1) {
        int gt911_int[] = {pinfo->int_pin};
        if (aw_gpio_pin_request("gt911_int",
                                 gt911_int,
                                 AW_NELEMENTS(gt911_int)) == AW_OK) {
            aw_gpio_pin_cfg(pinfo->int_pin, AW_GPIO_OUTPUT | AW_GPIO_PULL_UP);
            aw_gpio_set(pinfo->int_pin, 0);
        } else {
            AW_INFOF(("GT911  GPIO INT pin Request Fail !\r\n"));
        }
    }
...
static void __gt911_ts_init(struct awbl_dev *pdev)
{
    ...
    chip_type[10] = '\0';
     ...
     /* 配置用户定义的坐标大小*/
     pinfo->cfg_table[1] = (uint8_t)(pinfo->x_usr_output_max & 0xFF);
     pinfo->cfg_table[2] = (uint8_t)((pinfo->x_usr_output_max >> 8) & 0xFF);
     pinfo->cfg_table[3] = (uint8_t)(pinfo->y_usr_output_max & 0xFF);
     pinfo->cfg_table[4] = (uint8_t)((pinfo->y_usr_output_max >> 8) & 0xFF);

     /* 更新并保存配置 */
     GT911_Send_Cfg(p_ts_dev, CFG_SAVE, pinfo->cfg_table);
...
static void __gt911_ts_inst_init2 (struct awbl_dev *p_dev)
{
    ...
    /* 测量X轴采样值  */
    p_ts_dev->ts_serv.drv_fun.pfn_measure_x = __gt911_ts_measure_x;
    /* 设置是否锁屏 */
    p_ts_dev->ts_serv.drv_fun.pfn_lock_set = __gt911_ts_lock;
    /* 是否按下 */
    p_ts_dev->ts_serv.drv_fun.pfn_is_touch = __gt911_ts_is_touch;
    /* 准备测量Y轴坐标  */
    p_ts_dev->ts_serv.drv_fun.pfn_active_y = __gt911_ts_active_y;
    /* 准备测量X轴坐标  */
    p_ts_dev->ts_serv.drv_fun.pfn_active_x = __gt911_ts_active_x;
    /* 得到的坐标值，不需要校准  */
