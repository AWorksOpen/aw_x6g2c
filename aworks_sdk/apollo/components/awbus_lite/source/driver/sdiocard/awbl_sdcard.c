...
#define __IMX10xx_NAND_DEVINFO_DECL(p_devinfo, p_dev) \
    struct awbl_nand_mmc_dev_info *p_devinfo = \
        (struct awbl_nand_mmc_dev_info *)AWBL_DEVINFO_GET(p_dev)

#define __IMX10xx_SDIO_MEM_TO_CARD_DEV_DECL(p_sd_card,p_sd_mem)\
    struct awbl_sdcard_dev *p_sd_card = \
        (struct awbl_sdcard_dev *)p_sd_mem->p_drv
...
aw_local aw_err_t __imx10xx_nand_mmc_nvram_get(struct awbl_dev *p_dev,
                                               char            *p_name,
                                               int              unit,
                                               char            *p_buf,
                                               int              offset,
...
aw_local aw_err_t __imx10xx_nand_mmc_nvram_set(struct awbl_dev *p_dev,
                                               char            *p_name,
                                               int              unit,
                                               char            *p_buf,
                                               int              offset,
...
aw_local aw_err_t __imx10xx_nand_mmc_nvram_get (struct awbl_dev *p_dev,
                                                ...
                                                int              len)
{
    struct awbl_sdio_mem     *p_sd_mem = NULL;

    p_sd_mem = AW_CONTAINER_OF(p_dev,  struct awbl_sdio_mem,   super);

    __IMX10xx_SDIO_MEM_TO_CARD_DEV_DECL(p_sd_card,p_sd_mem);
    __IMX10xx_NAND_DEVINFO_DECL(p_devinfo, p_dev);
    ...
    for (i = 0; i < p_devinfo->seglst_count; i++) {
        if ((p_seg->unit == unit) && (strcmp (p_seg->p_name, p_name) == 0)) {
            sector_size = p_sd_mem->mem_info.csd.sector_size;
            /* 传输长度必须为扇区的整数倍 */
            copylen = len % sector_size;
            if (copylen != 0){
                aw_kprintf("len must be a multiple of %d\n", sector_size);
                return -AW_EIO;
            }

            len = len / sector_size; /* 块个数 */
            copylen = len;
            ...
            ret = awbl_sdcard_rw_sector ((struct awbl_sdcard_dev *) p_sd_card,
...
aw_local aw_const struct awbl_dev_method __g_sdcard_methods[] = {
        ...
        AWBL_METHOD(awbl_nvram_get, __imx10xx_nand_mmc_nvram_get),
        AWBL_METHOD(awbl_nvram_set, __imx10xx_nand_mmc_nvram_set),
        AWBL_METHOD_END
};
