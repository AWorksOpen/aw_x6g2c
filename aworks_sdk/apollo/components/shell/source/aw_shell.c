...
static const struct aw_shell_cmd *_cmdset_cmd_find (
        ...
        const char                *name,
        const char               **cmd_arg);
static const struct aw_shell_cmd *_cmdset_defcmd (
        struct aw_shell_cmdset   *cmdset,
        const char                *name,
        const char               **cmd_arg);
                              ...
                              uint8_t                id)
{
    ...
    i = 0;
    tmp = str;
    maxlen = p_this->item_max_len[id] - 1;
    while (tmp[i] != 0) {
        tmp         = tmp + i;
        last_space  = 0;
        for (i = 0; i < maxlen; i++) {
                              ...
                              struct aw_shell_user *p_user)
{
    ...
    while (p_stat != NULL) {
        info.ret = p_user->io_try_getc(p_user);
        ch = info.ret;
                ...
                if (NULL == cmd_find) {
                    ...
                    if (NULL != cmdset) {
                        ...
                        if (*args == '\0') {
                            ...
                            if (!cmd_find) {
                                /* The match failed. Try using the default command. */
                                cmd_find = _cmdset_defcmd(cmdset, args, (const char **) &args);
                            }
                        }
                    }
                }
        ...
        while (list != NULL) {
            for (cmd = list->start; cmd < list->end; cmd++) {
                ...
                if (cmd->desc) {
                    ...
                    aw_shell_chart_line(&chart, " ", desc_id, cur_line);
                }
...
{
    struct aw_shell_context *sc = sh->sc;
    int ret;
        ...
        const char               **cmd_arg)
{
    ...
    int                         len;
    const char                 *args;
    const struct aw_shell_cmd  *def_cmd = NULL;

    list = &cmdset->cmds;

    for (cmd = list->start; cmd < list->end; cmd++) {
        /* Obtain default command. */
        if (!def_cmd && cmd->name == NULL) {
            def_cmd = cmd;
            continue;
        }
