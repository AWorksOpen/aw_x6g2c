...
aw_local void play_wav_proc(struct wav_play *p_wav_play)
{
    int                         ret,buf_len;
    play_buffer_t              *p_buf;
    awbl_imx6ul_sai_buffer_t    sai_buf;
    ...
    while(1) {
        ...
        ret = aw_imx6ul_sai_tx_append_buffer(0,
                &sai_buf);
        if (ret != AW_OK) {
            goto cleanup;
        }
        ...
        void *p_arg)
{
    ...
    int                         mqs_enabled = 0;
    awbl_imx6ul_sai_config_t    config = {0};
    awbl_imx6ul_sai_buffer_t    sai_buf = {0};
    ...
    ret = aw_imx6ul_sai_tx_config(0,&config);
    if (ret != AW_OK) {
        goto cleanup;
    }

    /* enable the MQS */
    ret = aw_imx6ul_mqs_enable(p_wav_play->fmt.samples_per_sec,
...
aw_local void __imx6ul_mqs_trans_sync_complete (void *p_arg)
{
    if (p_arg) {
        aw_semb_give(p_arg);
    }
...
/*******************************************************************************
  public functions
*******************************************************************************/
aw_err_t  aw_imx6ul_sound_wav_play_async (const uint8_t   *p_wav_buf,
                                          size_t           len,
                                          aw_pfuncvoid_t   pfn_complete,
                                          void            *p_arg)
{
...
aw_err_t  aw_imx6ul_sound_wav_file_play_async (const char *filename,
                                          aw_pfuncvoid_t   pfn_complete,
                                          void            *p_arg)
{
    aw_err_t        ret = AW_OK;
...
aw_err_t  aw_imx6ul_sound_wav_play_sync (const uint8_t *p_wav_buf, size_t len)
{
    ...
    ret = aw_imx6ul_sound_wav_play_async(p_wav_buf,
                                         len,
                                         __imx6ul_mqs_trans_sync_complete,
                                        (void *)sem_id);
...
aw_err_t  aw_imx6ul_sound_wav_file_play_sync (const char *filename)
{
    AW_SEMB_DECL(sem_sync);
    ...
    ret = aw_imx6ul_sound_wav_file_play_async(filename,
                                         __imx6ul_mqs_trans_sync_complete,
                                        (void *)sem_id);
