 ...
 *
 * \brief 注意：
 * 当前demo同一时间只能打开一个USB转串口设备，
 * 关闭之后才可以打开别的USB转串口设备
 *
 * \brief 命令说明
 * userial_mem             检查当前USB转串口驱动的内存申请释放状态 
 * userial_detect          检测当前存在的USB转串口设备，会返回设备号和端口数量
 * userial_open            打开一个USB转串口设备，需要参数设备号和接口号，接口号输入-1默认是
 *                         第一个接口
 * userial_close           关闭一个USB转串口设备 
 * userial_config_get      获取当前USB转串口设备配置
 * userial_config_set      设置USB转串口配置，需要输入四个配置参数，波特率，数据位，
 *                         停止位和奇偶校验，奇偶校验参数(odd,even,none)
 * userial_timeout_get     获取传输超时时间，需要参数in/out，in代表接收超时，out代表发送超时
 * userial_timeout_set     设置传输超时时间，需要两个参数，第一个是
 *                         in/out，in代表接收超时，out代表发送超时，第二个是超时时间
 * userial_tranfersize_get 获取单次传输数据大小，需要参数in/out，in代表接收，out代表发送
 * userial_tranfersize_set 设置单次传输数据大小，需要两个参数，第一个是in/out，in代表接收，out代表发送
 *                         第二个是要设置的传输大小
 * userial_send            发送数据，需要输入要发送的数据，数据不宜过长，因为shell参数有限制大小
 * userial_read            接收数据，需要输入两个参数，第一个是接收缓存大小，第二个是异步接收或同步接收(sync/async)
 * userial_stop            停止接收数据
 * userial_wr_auto         自动收发，此时PC串口助手发送什么数据，USB转串口设备就会回发什么数据
 *                         需要参数(on/off)
 */
#include "Aworks.h"
#include "aw_delay.h"
