...
__restart:
    ...
    /* 发送英文短信*/
    for(i = 0;i < 3;i++){
        ret = aw_gprs_sms_send(gprs_id, (uint8_t *)sms_en_buf, strlen(sms_en_buf), TEST_PHONE_NUM,AW_GPRS_SMS_CODE_8BIT);
        if (ret != AW_OK) {
    ...
    /* 发送中文短信*/
    for(i = 0;i < 3;i++){
        ret = aw_gprs_sms_send(gprs_id, (uint8_t *)sms_ch_buf, strlen(sms_ch_buf), TEST_PHONE_NUM,AW_GPRS_SMS_CODE_8BIT);
        if (ret != AW_OK) {
            aw_kprintf("aw_gprs_sms_send error:%d\r\n", ret);
            aw_mdelay(1000);
        }
    ...
    /* 读取短信*/
    for(i = 0;i < 3;i++){
        ret = aw_gprs_sms_recv(gprs_id, p_sms, AW_GPRS_SMS_CODE_UTF8);
        if(ret != AW_OK){
